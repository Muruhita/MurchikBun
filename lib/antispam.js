...
const CONFIG = {
  ...
  BAN_DURATION: 60 * 60 * 24 * 4 // Бан на 7 дней
};
...
export async function checkSpam(userId, username) {
  ...
  if (attempts > CONFIG.MAX_ATTEMPTS) {
    ...
    const banData = JSON.stringify({
      ...
      reason: `Превышен лимит заявок: ${CONFIG.MAX_ATTEMPTS} в час. Автобан на 4 дня.`,
      timestamp: Date.now(),
      source: 'autospam'
    });
    ...
    return {
      isSpam: true,
      isBanned: true,
      message: `⛔ Вы отправили более ${CONFIG.MAX_ATTEMPTS} заявки за 1 час. Доступ заблокирован на 4 дня.`
    };
  }
